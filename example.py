...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='PyTorch SNN MNIST Example')
    parser.add_argument('--batch_size', type=int, default=64, metavar='N', help='input batch size for training (default: 64)')
    parser.add_argument('--test_batch_size', type=int, default=200, metavar='N', help='input batch size for testing (default: 1000)')
    parser.add_argument('--epochs', type=int, default=100, metavar='N', help='number of epochs to train (default: 100)')
    parser.add_argument('--optim', type=str, default='SGD', help='optimizer')
    parser.add_argument('--lr', type=float, default=1e-1, metavar='LR', help='learning rate (default: 0.01)')
    ...
    parser.add_argument('--load_model', action='store_true', default=False, help='For Loading the current Model')
    parser.add_argument('--ckpt_name', type=str, help='Checkpoint file name')
    ...
    checkpoint_path = './checkpoint/cifar10/'
    if os.path.isdir(checkpoint_path) and args.load_model:
        checkpoint = torch.load(checkpoint_path+args.ckpt_name+'.pt')
        model.load_state_dict(checkpoint+args.ckpt_name+'.pt')
        print('Model loaded.')
    ...
    if (args.save_model):
        torch.save(model.state_dict(), './checkpoint/cifar10/'+args.ckpt_name+'.pt')
        torch.save(model, './checkpoint/cifar10/'+args.ckpt_name+'.pth')
